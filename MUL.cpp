...
int MUL::excute(int inst, Data_mem &data, bool& on)
{
	...
	int int2 = data.getVar(operation.at(1).getValue());
	int result =int1*int2 ; 
	data.setVar(operation.at(2).getValue(),result );

	    printf("The SIM just multiplied memory location %d with value %d & memory location %d with value %d and stored it in memory location %d now with value %d \n", 
